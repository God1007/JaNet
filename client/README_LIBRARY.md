...
WeakNet客户端动态库 (`libweaknet.so`) 提供了一个C接口，供其他应用程序调用WeakNet gRPC服务的功能。
...
WEAKNET_GRPC_ADDRESS=${WEAKNET_GRPC_ADDRESS:-127.0.0.1:50051} \
./user_program
```
...
4. **线程安全**: 库内部使用gRPC stub和后台事件流线程
5. **环境依赖**: 需要WeakNet gRPC服务器运行
