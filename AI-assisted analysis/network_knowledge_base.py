#!/usr/bin/env python3
"""
文件职责：集中维护 RAG 和规则分析共用的静态网络知识，包括阈值、症状与排障建议。
这里的数据是人工配置字典，不包含训练、检索或模型调用逻辑。
"""

# 顶层 key 是检索类别；下层字段同时服务于 prompt 拼装和本地规则报告。
...
def get_network_knowledge():
    """返回完整知识库对象；调用方当前直接读取，不创建防御性副本。"""
    return NETWORK_KNOWLEDGE_BASE

# 将指定指标和值与对应知识条目组合，供上层分析器消费。
def analyze_metric(metric_type, value, context=""):
    """按类别查找知识并包装输入值；该函数只组织数据，不执行阈值判断。"""
