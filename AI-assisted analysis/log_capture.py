#!/usr/bin/env python3
"""
文件职责：启动 ``weaknet-grpc-server`` 子进程，将原始标准输出筛选、解析并格式化为 RAG 可消费的指标日志。
该模块只做文本适配，不保存历史数据，也不执行网络质量推理。
"""
...
# 启动 WeakNet 服务端，筛选原始日志并转换为 RAG 解析器约定的指标文本。
def capture_network_logs():
    """管理服务端子进程生命周期，并持续把识别到的监控日志输出到当前终端。"""
    print("🚀 启动WEAK_NET服务器...")
    
    try:
        # 这里会新启动一个服务端实例，而不是连接已经运行的进程；工作目录决定相对资源路径。
        ...
        # 每个正则对应服务端的一类日志契约；格式变化时需要同步更新这里和下游解析器。
        patterns = {
            # RTT信息: iface=eth0 rtt=1ms quality=1 state=1
            'rtt': re.compile(r'iface=(\w+) rtt=(-?\d+(?:\.\d+)?)ms quality=(\d+) state=(\d+)'),
            
            # RTT监控: RTT_MONITOR: eth0 | RTT: 1ms | Quality: 1 | Using: YES | Target: 223.5.5.5
            'rtt_monitor': re.compile(r'RTT_MONITOR: (\w+) \| RTT: (-?\d+(?:\.\d+)?)ms \| Quality: (\d+) \| Using: (\w+) \| Target: ([\d.]+)'),
            ...
            'interface_summary': re.compile(r'ACTIVE: (\w+) \| RTT: (-?\d+(?:\.\d+)?)ms \| Quality: (\d+) \| RSSI: (-?\d+)dBm \| TCP Loss: (-?[\d.]+)% \(([^)]*)\) \| Traffic: ([\d.]+)MB/s, (\d+) flows, (\d+) pps'),
            
            # 接口不活跃: INACTIVE: eth0 | RTT: -1ms | Quality: 0 | RSSI: -1000dBm | TCP Loss: -1% ()
            'interface_inactive': re.compile(r'INACTIVE: (\w+) \| RTT: (-?\d+(?:\.\d+)?)ms \| Quality: (\d+) \| RSSI: (-?\d+)dBm \| TCP Loss: (-?[\d.]+)% \(([^)]*)\)'),
        ...
        # 持续阻塞读取服务端 stdout；每行按首次命中的指标模式转换后立即输出。
        line_count = 0
        while True:
            ...
            if line:
                ...
                # 先做关键词粗过滤，减少后续逐个正则匹配的开销，同时保留错误诊断日志。
                if any(keyword in line for keyword in [
                    ...
                    # 匹配按“更具体格式优先”的顺序执行；成功后 continue，避免一行被重复归类。
                    # 解析并显示RTT信息
    ...
    # 用户中断和运行异常都进入 finally，保证子进程不会残留。
    except KeyboardInterrupt:
        print("\n⏹️ 日志截取已停止")
    except Exception as e:
        print(f"❌ 错误: {e}")
    finally:
        # 无论正常停止还是发生异常，都回收本函数创建的服务端子进程，避免遗留占用端口。
        if 'process' in locals():
            process.terminate()
            process.wait()

if __name__ == "__main__":
    # 作为脚本执行时持续采集；作为模块导入时不产生进程副作用。
